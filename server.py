...
class OrbeetoServer:
    ...
    def acknowledge_client(self, addr: tuple[str, int]) -> None:
    ...
    def send_client_connection_acknowledgement(self, addr: tuple[str, int]) -> None:
        ...
        packed = msgpack.packb(ack, use_bin_type=True)
        for _ in range(0, 100):
            self.udp_socket.sendto(packed, addr)
    ...
    def tick(self) -> None:
        ...
        match unpacked_data["action"]:
            case "connection_request":
                self.acknowledge_client(addr)
                self.send_client_connection_acknowledgement(addr)
            case _:
                pass

        self.check_client_pings()

    def check_client_pings(self) -> None:
